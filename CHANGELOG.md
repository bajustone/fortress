...
### Changed
- **Plugin composition and method entry points are construction-fixed (security hardening).** After all plugin factories return, Fortress snapshots validated plugin membership and descriptor capabilities, including routes, hooks, middleware, wrappers, dependencies, models, scope rules, claims enrichers, principal resolvers, and direct callable entry points from returned method surfaces. Adding, removing, replacing, or reordering caller-owned declarations after construction can no longer change Fortress-controlled membership, dispatch, manifests, adapters, or direct method selection. Captured functions still use their original receivers, so closure- and receiver-owned state intentionally remains live; trusted method code that performs `this.sibling()`, `super`, or an external lookup can still observe later mutation. Published method facades intentionally do not preserve source or prototype identity, `instanceof`, accessor descriptors, or original function equality. Effective own and inherited accessors are materialized once during construction and must return callables.
- **Published runtime snapshots are readonly in TypeScript.** Endpoint and manifest containers, plugin entries and method slots, and the `resolvePlugin` binding now reflect their frozen or non-replaceable runtime contracts. Assignments that declarations previously accepted will now fail compilation, while existing reads, calls, concrete class and symbol inference, referenced schema objects, and receiver- or closure-owned state remain supported and mutable where documented.

### Security
- **Permissionless API-key routes now work as authenticated-only endpoints.** A route declaring `security: ['apiKey']` without an IAM permission accepts a subject produced by any configured plugin resolver because resolved credentials are not provenance-tagged. It returns 401 when no plugin resolves a subject, including for invalid, expired, or revoked API keys and for a bearer JWT alone because the core JWT fallback is suppressed. IAM runs only when the route declares a permission; credential scope rules narrow declared permissions and are not standalone permissions.
...
- **`security: ['basic']` without a permission is rejected at construction.** Fortress ships no HTTP Basic credential verifier, so such a route did not require Basic credentials and could be served unauthenticated while the route manifest labelled it `authenticated` — `fortress check:public-routes` therefore never reported it. `createFortress()` now fails fast. To migrate: declare a permission and resolve a subject for it (a credential plugin's `resolvePrincipal` can supply one), use `security: ['bearer']`, or use `security: ['none']` if the route is intentionally public. The OAuth client-authentication endpoints (`POST /oauth/token`, `/oauth/introspect`, `/oauth/revoke`) remain exempt because they verify client credentials inside their own handlers; the other OAuth protocol routes are not, since they never check a Basic credential. Relatedly, a self-managed OAuth protocol route (any route declaring `bearerKind: 'oauth'`) can no longer declare an IAM permission at all: those handlers own their OAuth protocol security (authentication or intentional public access) and bypass the Fortress principal/RBAC pipeline, so the permission would never be evaluated — declaring one provided false assurance of IAM protection and could leave the route open. Adding a permission is therefore the correct migration for an ordinary Basic route, but not for an OAuth protocol route. `classifyEndpoint` no longer reports a permissionless Basic route as `authenticated`; on an unassembled runtime it classifies `default-deny`. The `RouteClassification` union is unchanged.
- **The validated route set is published as a frozen snapshot.** `createFortress()` clones each endpoint's routing, auth, and response envelope, freezes it, and publishes those clones as `fortress.endpoints`; the cached `fortress.manifest` array and its entries are frozen as well. Two consequences are deliberate. Endpoint identity changes — a published endpoint is no longer the same object a plugin declared, nor the shared `authEndpoints`/`iamEndpoints` singleton — so code comparing endpoints by reference must compare `method`/`path`/`handler` instead. And in-place mutation of a published endpoint or manifest entry now throws in strict mode; routes must be declared differently rather than edited after construction. Validation schemas reached through `input`/`responses` remain shared by reference because schema identity binds `$ref` component resolution, so schema mutation is still possible and is a documented non-goal. A manifest built directly via `buildRouteManifest()` is the caller's own array and stays mutable.
- **`POST /iam/check` rejects undeclared body properties.** The permission-check body is now a closed `oneOf` over the subject-aware (`subject`) and legacy (`userId`) forms: exactly one of the two must be present, and unknown top-level properties are rejected with a 422 validation error instead of being ignored. Both branches had to be closed because an open branch lets a malformed counterpart disqualify one arm so the other matches alone, which would contradict the union. Callers sending extra top-level fields must drop them; caller-supplied `context` is still limited to `tenantId` and `request`.
...
### Fixed
- Enforce the TypeScript 5.0 declaration floor and Express 4/5 adapter compatibility in CI. After one clean build, exact TypeScript 5.0.4 now compiles both generated ESM `.d.ts` and Node16 CommonJS `.d.cts` consumer projects. A separate current-compiler package fixture mounts Fortress on a real `express@4.21.2` application using aliased `@types/express@4.17.23`, with trace-verified isolation to its nested `express-serve-static-core` v4 while the existing Express 5 fixture remains unchanged. These are dev-only declaration gates: no runtime dependency, public export, peer range, or adapter behavior changes.
- Validate npm's CommonJS declaration surface as a real Node16 consumer. A `.cts` contract now resolves and exercises all 29 public subpaths through `require.types`, alongside the unchanged ESM/source contracts. Export parity requires every subpath's `import.types`, `import.default`, `require.types`, and `require.default` to map to the exact `.d.ts`, `.js`, `.d.cts`, and `.cjs` artifacts for one tsup entry, including root `types`/`main` agreement. CI, release, npm publication, and prepublish checks compile both declaration branches after a clean tsup build; no runtime export behavior changes.
- Parse Fortress runtime component `$ref`s exactly instead of taking the final path segment. The component-definition bridge now accepts only local `#/components/schemas/<name>` refs; external refs, nested or other-local pointers, anchors, malformed escaping, non-string values, accessors, and invalid decoded component names fail lazily with contextual errors. Legal encoded names resolve under their decoded component key, while inherited `$ref` properties are ignored without invocation. The exact-`oneOf` validator applies root `$ref` and sibling constraints before retaining exactly-one-branch semantics, and `nullable()` selects its ref wrapper without reading accessors or inherited values. Valid unresolved component refs remain permissive, recursive resolution and same-name conflict checks are unchanged, emitted schemas are not rewritten, and no external dereferencing or network support is added. Direct raw JSON Schema validation, including schemas with their own `$defs`, remains unchanged.
