...
### Security
- Hardened the tenancy plugin: tenant schema names now use numeric ids, tenant context comes from verified JWT custom claims instead of `X-Tenant-Code`, PostgreSQL `search_path` is transaction-pinned with a bound parameter, and missing tenant context fails closed.
