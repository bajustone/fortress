...
### Security
- Remediated all five findings from the internal adversarial security review (`docs/security-review-internal-2026-06-08.md`):
  - **(High) data-isolation cross-scope write:** the scoped adapter's `create` merged the scope value as a *default* that caller data could override, letting an authenticated user write a row into another scope/tenant by setting the scope field in the payload. Scope defaults are now spread last (authoritative).
  - **(High) API-key outlived account deactivation:** API-key resolution and permission resolution only gated `isActive` for SERVICE_ACCOUNT subjects; a deactivated or deleted USER kept full access via an existing key. Both paths now enforce USER `isActive`/existence, matching the JWT login/refresh gates.
  - **(Medium) data-isolation cross-scope move:** the scoped adapter's `update` guarded only the `WHERE` clause, so a user could rewrite a row's scope field and move it to another scope. The scope field is now re-asserted on update.
  - **(Low) fail-open CSRF skip-path:** the standalone Hono `createCsrfMiddleware` matched skip paths with a bare `startsWith` (no segment boundary), so `/api/public` also skipped `/api/public-keys`. It now reuses the core segment-boundary matcher (`matchesSkipPath`, now exported). **Breaking:** skip paths match at segment boundaries — drop any trailing slash (`/webhook`, not `/webhook/`).
  - **(Low) RBAC cache TOCTOU:** the opt-in permission cache could re-store a stale ALLOW when a revocation raced an in-flight read-through, keeping a revoked subject authorized for the TTL. The cache now exposes a generation counter and the read-through skips the write if an invalidation occurred during the load.
