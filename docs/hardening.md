...
- **Verify:** `fortress check:public-routes --module ./fortress.config.ts` in CI catches a stray `.security('none')`; the bootstrap allow-list covers only Fortress's intentional public surface, so your own public routes need an explicit `--allow`. **`--module` is required to see your routes** — without it the command only reviews Fortress's own auth + IAM surface and will pass while your plugin leaks a public route.

### RBAC drift

- **Do:** Add `fortress manifest:check --module ./fortress.config.ts` to CI; also run `runFortressChecks({ fortress })` in a test that constructs your real `Fortress` instance.
...
- **Do:** Pin to a stable minor line (`@bajustone/fortress@~2.0`). Fortress follows semantic versioning; breaking API changes require a new major release.
