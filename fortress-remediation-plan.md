...
- **Phase 1 — ✅ COMPLETE (exit criteria met 2026-06-16).** All 13 work items are coded and correct, and the three verification tests below **have now landed** (see the implementation-progress note above). _The bullets below are retained as the record of what those tests cover._
  - **#5/#7/M68 (verified-email linking):** guards correct (`social-login/index.ts:513,518-519`); tests landed for `email_verified:false` blocking + the `isActive` guard. The latent Microsoft fail-open is **now also fixed** (fails closed — see watch-outs).
...
| ~~MED~~ **RESOLVED (2026-06-16)** | ~~Microsoft provider sets `emailVerified=true` when the claim is absent~~ — now **fails closed** (`providers/microsoft.ts`, absent ⇒ unverified, matching the other providers); tested in `providers.test.ts`, documented in SECURITY.md/CHANGELOG | `providers/microsoft.ts` | Done — opt back in via custom `mapProfile` |
...
> **[2026-06-16 status] ✅ DONE — exit criteria MET.** Implemented by `be1753f` (+ `8249a2b` HTTP routing, `39f1643`+`e6a9002` webhook v1); all 13 work items verified present and correct. The three outstanding regression tests **landed this session** — #5/#7 (`email_verified:false` blocked + `isActive` guard), #34/#35/M1 (window-expiry reset + cannot-re-lock + identifier normalization), #46 (second-bootstrap→403); full suite 1124 passing. The two former follow-ups are also closed: Microsoft now **fails closed** on an absent `email_verified` claim (tested + documented), and the **degraded-OIDC fail-closed regression test** is added. Phase 1 is fully complete.
...
- ~~**Microsoft `emailVerified` default**~~ **RESOLVED (2026-06-16): fail-closed.** `providers/microsoft.ts` now maps an absent `email_verified` to unverified (matching the other providers), so a Microsoft login no longer auto-satisfies the #5/#7 gate. Tested + documented (SECURITY.md, CHANGELOG). _(Phase 1 — closed.)_
